...
def ai_learn_writing_code_and_write_junit_test():
    ...
    for class_path in class_list:
        ...
        if len(class_content) < 9000:
            print(f'文件内容6000, 跳过: {class_path}')
            continue
...
def write_test_code(need_upload_file_list, messages, zhipu_url, headers, data, class_path, class_content_optimize, zhipu_language, zhipu_framework):
    print(f'开始写单元测试')
